...
return !element.dispatchEvent(evt);
	...
	def mouse_click(self):
		...
		`introduction` for details about locating elements.
		""" 
		ActionChains(self._current_browser()).move_by_offset(1042, 840).click()
	...
	def mouse_release(self, locator):
			...
			if element is None:
				raise AssertionError("ERROR: Element %s not found." % (locator))
			ActionChains(self._current_browser()).release(element)
		   
	def key_hold(self,keyvalue):
			"""Simulates pressing the left mouse button.
			"""
			ActionChains(self._current_browser()).key_down(keyvalue).perform()
